...
// Enhanced grammar and style correction rules
const grammarRules = [
  ...
  // File extension fixes (common in tech writing)
  { pattern: /\.STL\b/g, replacement: '.stl' },
  { pattern: /\.CTB\b/g, replacement: '.ctb' },
  { pattern: /\.JSON\b/g, replacement: '.json' },
  { pattern: /\.HTML\b/g, replacement: '.html' },
  { pattern: /\.CSS\b/g, replacement: '.css' },
  { pattern: /\.JS\b/g, replacement: '.js' },
  { pattern: /\.TS\b/g, replacement: '.ts' },
  ...
  // Markdown image spacing fixes
  { pattern: /\]\(/g, replacement: '](' }, // Fix space between ] and (
  { pattern: /!\s*\[/g, replacement: '![' }, // Fix space after !
  
  // Double space fixes
  { pattern: /\s{2,}/g, replacement: ' ' }, // Multiple spaces to single space
  
  // Sentence case after periods (but not after abbreviations or numbers)
  { pattern: /\.\s+([a-z])(?![a-z]*\.)/g, replacement: (match, p1) => '. ' + p1.toUpperCase() },
  ...
  { pattern: /\bazure\b(?!\s*openai)/gi, replacement: 'Azure' }, // Don't change "Azure OpenAI"
  ...
  { pattern: /\buv\b/gi, replacement: 'UV' },
  { pattern: /\bstl\b/gi, replacement: 'STL' },
  { pattern: /\bapi\b/gi, replacement: 'API' },
  { pattern: /\brest\b(?=\s+api)/gi, replacement: 'REST' },
  { pattern: /\bhttp\b/gi, replacement: 'HTTP' },
  { pattern: /\bhttps\b/gi, replacement: 'HTTPS' },
  { pattern: /\burl\b/gi, replacement: 'URL' },
  { pattern: /\bml\b(?=\s+(?:model|studio|service))/gi, replacement: 'ML' },
  { pattern: /\bai\b(?=\s+(?:service|model|api))/gi, replacement: 'AI' },
  
  // Improve readability
  { pattern: /\*([^*]+)\*(?!\s*$)/g, replacement: (match, p1) => `*${p1.trim()}*` }, // Trim spaces in italic text
  { pattern: /\*\*([^*]+)\*\*(?!\s*$)/g, replacement: (match, p1) => `**${p1.trim()}**` }, // Trim spaces in bold text
];

// Common spelling corrections for technical terms
const spellCheckRules = [
  { pattern: /\baccesible\b/gi, replacement: 'accessible' },
  { pattern: /\baccommodate\b/gi, replacement: 'accommodate' },
  { pattern: /\bachieve\b/gi, replacement: 'achieve' },
  { pattern: /\bacquire\b/gi, replacement: 'acquire' },
  { pattern: /\baddress\b/gi, replacement: 'address' },
  { pattern: /\boccur\b/gi, replacement: 'occur' },
  { pattern: /\boccurred\b/gi, replacement: 'occurred' },
  { pattern: /\boccurrence\b/gi, replacement: 'occurrence' },
  { pattern: /\bseparate\b/gi, replacement: 'separate' },
  { pattern: /\breceive\b/gi, replacement: 'receive' },
  { pattern: /\breceived\b/gi, replacement: 'received' },
  { pattern: /\bdefinitely\b/gi, replacement: 'definitely' },
  { pattern: /\benvironment\b/gi, replacement: 'environment' },
  { pattern: /\benvironments\b/gi, replacement: 'environments' },
];
...
function validateImageReferences(content, filePath) {
  const issues = [];
  const imageRegex = /!\[([^\]]*)\]\(([^)]+)\)/g;
  let match;
  ...
  while ((match = imageRegex.exec(content)) !== null) {
    ...
    if (!altText.trim()) {
      issues.push(`Missing alt text for image: ${imagePath}`);
    }
    
    // Check for relative paths that might be broken
    if (imagePath.startsWith('./') || imagePath.startsWith('../')) {
      issues.push(`Relative image path found: ${imagePath}`);
    }
    
    // Check for spaces in image paths (should be encoded)
    if (imagePath.includes(' ')) {
      issues.push(`Image path contains spaces: ${imagePath}`);
    }
  }
...
function validateLinks(content) {
  ...
  while ((match = linkRegex.exec(content)) !== null) {
    ...
    if (!linkText.trim()) {
      issues.push(`Empty link text for URL: ${url}`);
    }
    
    // Check for suspicious URLs
    if (url.includes('localhost') || url.includes('127.0.0.1')) {
      issues.push(`Local URL found: ${url}`);
    }
  }
...
function processMarkdownFile(filePath) {
  try {
    ...
    const imageIssues = validateImageReferences(markdownContent, filePath);
    const linkIssues = validateLinks(markdownContent);
    
    if (imageIssues.length > 0) {
      console.log(`  Image issues found:`);
    ...
    } else if (imageIssues.length === 0 && linkIssues.length === 0) {
      console.log(`✓ No changes: ${path.basename(filePath)}`);
    } else {
      console.log(`✓ Validation only: ${path.basename(filePath)}`);
    }
...
if (fs.existsSync(contentDir)) {
  console.log('Starting enhanced grammar correction and validation...\n');
  processDirectory(contentDir);
  console.log('\nEnhanced proofreading complete!');
} else {
