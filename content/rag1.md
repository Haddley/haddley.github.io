...
One real, easy-to-repeat mistake worth naming here: a plain Vite React project needs `@vitejs/plugin-react` (and a `vite.config.js` that loads it) for JSX to compile correctly. [Vite](https://vitejs.dev) is the build tool responsible for that JSX-to-`React.createElement`-calls translation mentioned earlier, plus serving the app during development and bundling it for production. Leave the plugin out, and Vite falls back to a JSX transform that emits `React.createElement(...)` calls without ever importing `React` — the page loads, the network tab looks fine, and the screen just stays blank, failing silently in the browser console rather than at build time, because the missing `React` reference only ever gets touched once the component actually tries to render, by which point there is no clear error message pointing back at the real, structural cause. Both files are two-minutes-each additions, and worth doing from the start rather than debugging a blank page later.
...
- **`StdioServerParameters` does not inherit the parent process's environment by default.** Without `env=dict(os.environ)`, the spawned `mcp_server.py` process has no `DATABASE_URL` at all, and crashes on import with a bare `KeyError` before ever reaching the MCP handshake — a real, documented gotcha, not a hypothetical one. The reason this trips people up is that most subprocess-launching tools *do* inherit the parent environment by default, so the opposite default here is an easy assumption to get wrong once and remember afterwards.
- **A tool returning `list[dict]` does not land as one JSON blob in `result.content[0].text`.** This particular SDK version (`mcp==2.0.0`) serializes each dict in the list as its own separate `TextContent` block instead — so `result.content` here is a list of three separate text blocks, one per search result, each one independently JSON-parseable but not a single combined array. `result.structured_content["result"]` sidesteps the issue entirely — it hands back the tool's actual return value, already parsed, under a `"result"` key, matching the function's own return type exactly, regardless of how many separate blocks the plain-text version was split into.
...
This is worth doing before ever pointing a real AI agent at a new MCP server: Inspector shows the exact JSON-RPC traffic (the message log on the right, timestamped, every `initialize`, `tools/list`, and `tools/call` round trip), which is the fastest way to catch a shape mismatch like the `list[dict]` one here before it becomes a confusing failure inside Claude Code instead, where the underlying protocol messages are not normally visible at all. JSON-RPC is the specific message format MCP is built on top of — every request carries a method name (`initialize`, `tools/call`, and so on) and an `id` used to match each response back to the request that triggered it, seen directly in the raw `initialize` example below.
