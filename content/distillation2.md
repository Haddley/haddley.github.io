...
That is not a general "use a lower learning rate" tip, and a control confirms it: I reran tiny's 100M-pretrain SFT stage at the same lowered rate, on the same checkpoint that produced this post's cleanest result. It got worse — perplexity 28.69, against 25.99 at the original rate — because tiny was never in the overfitting regime the lower rate is a remedy for. It just had less distance covered in the same 10,000 steps. The lower learning rate fixed a problem bigger specifically had; applied to a model that didn't have that problem, it is only a slower run.
...
- **Even the winning configuration still overfits during SFT, and the fix is specific, not general.** A 3x lower SFT learning rate produced both a better peak and a slower climb off it for bigger, but made tiny's own best result *worse* on the identical checkpoint and data. The lower rate was a remedy for a capacity/data mismatch bigger specifically had, not a free tuning win — applied where that mismatch doesn't exist, it is only a slower run.
