...
The training target so far has been a *one-hot* vector — a list with a single 1 in the slot for the correct next token and a 0 in every other slot, the same encoding the MNIST model used for its digit labels back in [Part 9](/posts/machinelearning9/). (The name comes from digital circuits, where exactly one wire in a group is held high.) It says only "the next word is *cat*".

A trained model has more to say than that. Before it picks a token it produces a *logit* for every entry in the vocabulary — a raw score, one number per possible token — and a softmax turns those scores into a probability distribution: "*cat* 0.6, *dog* 0.2, *kitten* 0.1, *the* 0.001, …". That distribution also tells you that *dog* and *kitten* were reasonable and *the* was not. **Logit distillation** trains the student to reproduce the teacher's whole distribution, not just its peak — a richer signal per token, and the reason a distilled small model can learn faster than the same model trained from scratch on raw text.
