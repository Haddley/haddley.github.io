export const CANVAS_WIDTH = 800;
export const CANVAS_HEIGHT = 600;
export const ALIEN_ROWS = 5;
export const ALIEN_COLS = 10;
export const ALIEN_WIDTH = 40;
export const ALIEN_HEIGHT = 30;
export const ALIEN_SPACING_X = 55;
export const ALIEN_SPACING_Y = 45;
export const ALIEN_SPEED_X = 1;
export const ALIEN_SPEED_Y = 10;
export const ALIEN_MAX_SPEED = 8;
export const MARCH_FREQ = [320, 260, 220, 190];
export const PLAYER_WIDTH = 50;
export const PLAYER_HEIGHT = 30;
export const PLAYER_SPEED = 5;
export const BULLET_WIDTH = 4;
export const BULLET_HEIGHT = 12;
export const PLAYER_BULLET_SPEED = 7;
export const ALIEN_BULLET_SPEED = 3;
export const ALIEN_SHOOT_INTERVAL = 1500;
export const LIVES = 3;
export const RESPAWN_DELAY = 1500;
export const INVULNERABLE_DURATION = 2000;
export const SHIELD_COUNT = 4;
export const SHIELD_BLOCK_SIZE = 6;
export const SHIELD_ROWS = 6;
export const SHIELD_COLS = 12;
export const SHIELD_WIDTH = SHIELD_BLOCK_SIZE * SHIELD_COLS;
export const SHIELD_HEIGHT = SHIELD_BLOCK_SIZE * SHIELD_ROWS;
export const UFO_WIDTH = 50;
export const UFO_HEIGHT = 20;
export const UFO_SPEED = 2;
export const UFO_MIN_INTERVAL = 15000;
export const UFO_MAX_INTERVAL = 25000;
export const UFO_Y = 50;
export const WAVE_TRANSITION_DURATION = 1500;
export const MAX_WAVE_DESCENT = 3;
export const EXPLOSION_DURATION = 400;
export const PLAYER_Y = CANVAS_HEIGHT - 50;
export const PLAYER_BULLET_Y = PLAYER_Y - PLAYER_HEIGHT;
export const HI_SCORE_KEY = 'invaders_hi_score';
